...
> Based on [this blog post](https://frontendmasters.com/blog/what-you-need-to-know-about-modern-css-2025-edition/#popovers-invokers)

Things to know:

1. Popovers are different from modals or dialogs. When visible, the rest of the page is still interactive.
2. There is a significant overlap between the `popover` and `dialog` elements.
3. **The main use-case are action menus or toast notifications**.
4. There are multiple ways to trigger the `popover`. Either via _invokers_ or JS APIs.

The following code will display the `popover` at the bottom of the viewport (centered).

**The _invokers_ API is not yet available in every browser**.
...
<html lang="en">
  ...
  <body>
    ...
    <div id="popover" popover="manual">
      <p>Content</p>
      <button commandfor="popover" command="hide-popover">Close</button>
    </div>
  </body>
</html>
```
