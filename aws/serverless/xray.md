...
The _AWS X-Ray SDK_ does not send the traces directly to _AWS X-Ray service_. It communicates with a background process (the daemon).
This process is the one that communicates with _AWS X-Ray service_ and sends the traces. The [official _AWS X-Ray_ documentation explains the notion of a daemon very well](https://docs.aws.amazon.com/xray/latest/devguide/xray-daemon.html).
...
Since the unit/integration tests most likely could be run locally on your machine _AWS X-Ray_ will not automatically generate that "root" segment in that case.
...
I recommend you do the latter. [Please refer to the documentation in regards to the implementation](https://github.com/aws/aws-xray-sdk-node/tree/master/packages/core#context-missing-strategy-configuration).
...
Usually, this is a good thing. We would not want to spend too much data to _X-Ray_. The trace indigestion costs money!
...
> The default sampling rule is one request per second and 5 percent of additional requests. This sampling rate cannot be configured for Lambda functions.

### Still nothing

Did you configure the sampling? Still nothing?

- **Depending on the service you are using, you might need to configure IAM**.
  The `xray:PutTelemetryRecords` and `xray:PutTraceSegments` are required for the service to communicate with its backend correctly.
...
An excellent example of what I'm talking about is **the `DocumentClient`**. [Refer to this GitHub thread](https://github.com/aws/aws-xray-sdk-node/issues/23) for implementation details.
...
You can read more about this issue (and other issues) in [this excellent blog post](https://theburningmonk.com/2017/06/aws-x-ray-and-lambda-the-good-the-bad-and-the-ugly/).

## Custom sampling

It is entirely possible to configure the sampling rules. **The sampling rules can even be configured PER network request path basis**.
Having so much control is neat. I'm kind of disappointed in myself that I was not aware of the controls _X-Ray_ exposes.

Here is a snippet of how one might change **HTTP sampling rules** for Node.Js based application.
...
If you are unsure what, _X-Ray daemon_ is, refer to the _X-Ray daemon_ section.
...
From the observability perspective, it is vital to have a holistic view of what is happening inside your application.
As we all know, network operations might fail. In such a case, we usually retry the requests.

_AWS X-Ray_ is not perfect in this department. You can visualize some of the retries, but not on all occasions.
...
You will see in the **console the status from the first request – a "fault"**. This is very unfortunate and misleading.

What if I want to see traces for all the requests, even the ones that were a result of a retry?
...
With the _trace header_ not present when making the request, _AWS X-Ray_ will be forced to create a new one.
**This ensures that no matter what happens, the retried request will be traced**.

## The missing X-Ray data (AWS Lambda)

You develop your app. It's ready. You deploy it. All seems good. You check the logs, and you see the following message:
...
The most important thing to know is that **some functionalities of _AWS X-Ray_ cannot be used outside of the _AWS Lambda_ handler**.

- You cannot retrieve the segment outside of the handler.

  ```js
  import AWSXray from "aws-xray-sdk";
  // Will cause an error log.
...
- You cannot create custom subsegments outside of the handler.

  ```js
  import AWSXray from "aws-xray-sdk";
  // Will cause an error log.
  const data = AWSXray.captureFunc("init", function init(subsegment) {
  /**
   * Do we initialization.
   */
  subsegment?.close();
  });
...
With constraints from above in mind, you might be wondering: **what does _outside_ of the _AWS Lambda_ handler mean?**.
It means **before your handler was invoked by our function runtime**.

Remember that **using _AWS X-Ray_ in your handler middleware functions is safe**. With middlewares, your handler is the whole chain of calls, not the last function that is invoked.
